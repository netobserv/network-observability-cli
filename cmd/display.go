...
const (
	...
	defaultFramesPerSecond = 30  // frames per second
...
func createDisplay() {
	app = tview.NewApplication().SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		switch event.Key() {
		case tcell.KeyCtrlC:
...
func addFlow(genericMap config.GenericMap) {
...
func updateScreen() {
	mainView.ResizeItem(tableView, 0, 1)
}
