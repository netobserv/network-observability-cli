...
func TestFlowTableDefaultDisplay(t *testing.T) {
	...
	assert.Equal(t, `Time              SrcName                                        SrcType   DstName                                        DstType   DropBytes     DropPackets   DropState             DropCause                                 DnsId   DnsLatency  DnsRCode  DnsErrno  RTT     OvsMonitorMDs     `, rows[0])
...
func TestFlowTableAdvancedDisplay(t *testing.T) {
	...
	rows := getRows([]string{pktDropDisplay, dnsDisplay, rttDisplay, ovsDisplay}, []string{noEnrichment})
	assert.Equal(t, 4, len(rows))
	assert.Equal(t, `Time              SrcAddr                                   SrcPort  DstAddr                                   DstPort  DropBytes     DropPackets   DropState             DropCause                                 DnsId   DnsLatency  DnsRCode  DnsErrno  RTT     OvsMonitorMDs     `, rows[0])
	...
	// set display to OVS
	rows = getRows([]string{ovsDisplay}, []string{noEnrichment})
	assert.Equal(t, 4, len(rows))
	assert.Equal(t, `Time              SrcAddr                                   SrcPort  DstAddr                                   DstPort  OvsMonitorMDs     `, rows[0])
